...
public sealed class TagsTests(DevRecordWebAppFactory factory) : IntegrationTestFixture(factory)
{
    [Fact]
    ...
    public async Task CreateTag_ShouldFail_WhenMaxTagsLimitReached()
    {
        ...
        for (int i = 1; i <= tagsOptions.MaxAllowedTags; i++)
        {
            ...
            HttpResponseMessage createResponse = await client.PostAsJsonAsync(Routes.Tags.Create, dto);
            Assert.Equal(HttpStatusCode.Created, createResponse.StatusCode); // verify each creation succeeded
        }
