...
public sealed class CreateEntryBatchDtoValidatorTests
{
    ...
    [Fact]
    public async Task Validate_ShouldNotReturnError_WhenEntriesIsExactlyMaxBatchSize()
    {
        var dto = new CreateEntryBatchDto
        {
            Entries = [..Enumerable.Range(0, 20)
                .Select(_ => new CreateEntryDto
                {
                    HabitId = Habit.NewId(),
                    Value = 1,
    ...
    [Fact]
    public async Task Validate_ShouldReturnError_WhenMultipleEntriesAreInvalid()
    {
        var dto = new CreateEntryBatchDto
        {
            Entries =
            [
                new CreateEntryDto { HabitId = string.Empty, Value = 1, Date = DateOnly.FromDateTime(DateTime.UtcNow) },
