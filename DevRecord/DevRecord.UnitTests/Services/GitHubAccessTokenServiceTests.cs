...
public sealed class GitHubAccessTokenServiceTests : IDisposable
{
    ...
    [Fact]
    public async Task GetAsync_ShouldReturnToken_WhenTokenIsExpired()
    {
        const string userId = "user123";
        var expiredToken = new GitHubAccessToken
        {
            Id = GitHubAccessToken.NewId(),
            UserId = userId,
            Token = _encryptionService.Encrypt("github-token"),
        ...
        await _dbContext.GitHubAccessTokens.AddAsync(expiredToken);
        await _dbContext.SaveChangesAsync();

        string? result = await _gitHubAccessTokenService.GetAsync(userId);

        // Document the current behaviour — does it return expired tokens?
        Assert.NotNull(result); // or Assert.Null if it should filter expired
    }
